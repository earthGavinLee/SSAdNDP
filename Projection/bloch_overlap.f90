...
!$OMP DO SCHEDULE(STATIC)
   DO ik=1,nkpts
      ...
      IF( SVD )THEN
         ...
         CALL GESVD(bloch_s_inv(:,:,ik),sing_value,u_matrix,v_matrix,work_array,'A')
...
SUBROUTINE check_linear_depend(input_mat,eig_min)
  ...
  CALL HEEV(test_mat,eigval,'V','U',INFO)
